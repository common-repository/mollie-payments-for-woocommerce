...
class AssetsModule implements ExecutableModule
{
    ...
    public function enqueueBlockCheckoutScripts(Data $dataService, array $gatewayInstances): void
    {
        ...
        \Mollie\WooCommerce\Assets\MollieCheckoutBlocksSupport::localizeWCBlocksData($dataService, $gatewayInstances);
    }
    ...
    {
        wp_register_script('babel-polyfill', $this->getPluginUrl($pluginUrl, '/public/js/babel-polyfill.min.js'), [], (string) filemtime($this->getPluginPath($pluginPath, '/public/js/babel-polyfill.min.js')), \true);
        ...
        wp_register_script('mollie-components', $this->getPluginUrl($pluginUrl, '/public/js/mollie-components.min.js'), ['underscore', 'jquery', 'mollie', 'babel-polyfill'], (string) filemtime($this->getPluginPath($pluginPath, '/public/js/mollie-components.min.js')), \true);
        wp_register_script('mollie-components-blocks', $this->getPluginUrl($pluginUrl, '/public/js/mollie-components-blocks.min.js'), ['underscore', 'jquery', 'mollie', 'babel-polyfill'], (string) filemtime($this->getPluginPath($pluginPath, '/public/js/mollie-components-blocks.min.js')), \true);
    ...
    {
        wp_register_script('mollie_block_index', $this->getPluginUrl($pluginUrl, '/public/js/mollieBlockIndex.min.js'), ['wc-blocks-registry', 'underscore', 'jquery'], (string) filemtime($this->getPluginPath($pluginPath, '/public/js/mollieBlockIndex.min.js')), \true);
    ...
    {
        if (is_admin() || !mollieWooCommerceIsCheckoutContext() && !has_block("woocommerce/checkout")) {
            return;
        }
        try {
    ...
    protected function setupModuleActions(ContainerInterface $container): void
    {
        ...
        add_action('woocommerce_blocks_loaded', function () use ($dataService, $gatewayInstances, $pluginUrl, $pluginPath, $hasBlocksEnabled) {
            if ($hasBlocksEnabled && is_admin() && class_exists('Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType')) {
                add_action('woocommerce_blocks_payment_method_type_registration', function (PaymentMethodRegistry $paymentMethodRegistry) use ($dataService, $gatewayInstances, $pluginUrl, $pluginPath) {
                    $paymentMethodRegistry->register(new \Mollie\WooCommerce\Assets\MollieCheckoutBlocksSupport($dataService, $gatewayInstances, $this->getPluginUrl($pluginUrl, '/public/js/mollieBlockIndex.min.js'), (string) filemtime($this->getPluginPath($pluginPath, '/public/js/mollieBlockIndex.min.js'))));
                });
            }
        });
        add_action('init', function () use ($container, $hasBlocksEnabled, $settingsHelper, $pluginUrl, $pluginPath, $dataService) {
