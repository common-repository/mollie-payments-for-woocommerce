...
class MollieSettingsPage extends WC_Settings_Page
{
    ...
    public function __construct(Settings $settingsHelper, string $pluginPath, array $gateways, array $paymentMethods, bool $isTestModeEnabled, Data $dataHelper)
    {
    ...
    public function get_settings($currentSection = '')
    {
        ...
        if ('mollie_components' === $currentSection) {
            $mollieSettings = $this->sectionSettings($this->componentsFilePath());
        }
         ...
         */
        $mollieSettings = apply_filters(self::FILTER_COMPONENTS_SETTINGS, $mollieSettings);
    ...
    public function addGlobalSettingsFields(array $settings): array
    {
        ...
        $presentationText .= '<p>' . __('Please go to', 'mollie-payments-for-woocommerce') . '<a href="https://my.mollie.com/dashboard/signup?utm_source=woocommerce&utm_medium=plugin&utm_campaign=partner">' . __(' the signup page', 'mollie-payments-for-woocommerce') . '</a> ';
        ...
        $presentation = '' . '<div style="width:1000px"><div style="font-weight:bold;"><a href="https://help.mollie.com/hc/en-us/sections/12858723658130-Mollie-for-WooCommerce">' . __('Plugin Documentation', 'mollie-payments-for-woocommerce') . '</a> | <a href="https://www.mollie.com/contact/merchants">' . __('Contact Support', 'mollie-payments-for-woocommerce') . '</a></div></div>' . '<span></span>' . '<div id="" class="" style="width: 1000px; padding:5px 0 0 10px"><p>' . $presentationText . '</p></div>';
        $content = '' . $presentation . $this->settingsHelper->getPluginStatus() . $this->getMollieMethods();
        ...
        $debugDesc .= ' ' . sprintf(
            ...
            __('Log files are saved to <code>%s</code>', 'mollie-payments-for-woocommerce'),
            defined('Mollie\WC_LOG_DIR') ? WC_LOG_DIR : WC()->plugin_path() . '/logs/'
        );
        // Global Mollie settings
        $mollieSettings = [['id' => $this->settingsHelper->getSettingId('title'), 'title' => __('Mollie Settings', 'mollie-payments-for-woocommerce'), 'type' => 'title', 'desc' => '<p id="' . $this->dataHelper->getPluginId() . '">' . $content . '</p>' . '<p>' . __('The following options are required to use the plugin and are used by all Mollie payment methods', 'mollie-payments-for-woocommerce') . '</p>'], ['id' => $this->settingsHelper->getSettingId('live_api_key'), 'title' => __('Live API key', 'mollie-payments-for-woocommerce'), 'default' => '', 'type' => 'text', 'desc' => sprintf(
            /* translators: Placeholder 1: API key mode (live or test). The surrounding %s's Will be replaced by a link to the Mollie profile */
            __('The API key is used to connect to Mollie. You can find your <strong>%1$s</strong> API key in your %2$sMollie account%3$s', 'mollie-payments-for-woocommerce'),
            'live',
            ...
            '</a>'
        ), 'css' => 'width: 350px', 'placeholder' => __('Live API key should start with live_', 'mollie-payments-for-woocommerce')], ['id' => $this->settingsHelper->getSettingId('test_mode_enabled'), 'title' => __('Enable test mode', 'mollie-payments-for-woocommerce'), 'default' => 'no', 'type' => 'checkbox', 'desc_tip' => __('Enable test mode if you want to test the plugin without using real payments.', 'mollie-payments-for-woocommerce')], ['id' => $this->settingsHelper->getSettingId('test_api_key'), 'title' => __('Test API key', 'mollie-payments-for-woocommerce'), 'default' => '', 'type' => 'text', 'desc' => sprintf(
            /* translators: Placeholder 1: API key mode (live or test). The surrounding %s's Will be replaced by a link to the Mollie profile */
            __('The API key is used to connect to Mollie. You can find your <strong>%1$s</strong> API key in your %2$sMollie account%3$s', 'mollie-payments-for-woocommerce'),
            'test',
            '<a href="https://my.mollie.com/dashboard/developers/api-keys?utm_source=woocommerce&utm_medium=plugin&utm_campaign=partner" target="_blank">',
            '</a>'
        ), 'css' => 'width: 350px', 'placeholder' => __('Test API key should start with test_', 'mollie-payments-for-woocommerce')], ['id' => $this->settingsHelper->getSettingId('debug'), 'title' => __('Debug Log', 'mollie-payments-for-woocommerce'), 'type' => 'checkbox', 'desc' => $debugDesc, 'default' => 'yes'], ['id' => $this->settingsHelper->getSettingId('sectionend'), 'type' => 'sectionend']];
    ...
    public function getMollieMethods(): string
    {
        ...
        $iconAvailable = ' <span style="color: green; cursor: help;" title="' . __('Gateway enabled', 'mollie-payments-for-woocommerce') . '">' . strtolower(__('Enabled', 'mollie-payments-for-woocommerce')) . '</span>';
        $iconNoAvailable = ' <span style="color: red; cursor: help;" title="' . __('Gateway disabled', 'mollie-payments-for-woocommerce') . '">' . strtolower(__('Disabled', 'mollie-payments-for-woocommerce')) . '</span>';
        $content .= '<br /><br />';
        ...
        $content .= ' (<a href="' . esc_url($refresh_methods_url) . '">' . strtolower(__('Refresh', 'mollie-payments-for-woocommerce')) . '</a>)';
        $content .= '<ul class="mol-responsive-table-3">';
        $mollieGateways = $this->registeredGateways;
        //this are the gateways enabled
        ...
        foreach ($paymentMethods as $paymentMethod) {
            ...
            if ($paymentMethodEnabledAtMollie) {
                ...
                $content .= ' <a href="' . $this->getGatewaySettingsUrl($gatewayKey) . '">' . strtolower(__('Edit', 'mollie-payments-for-woocommerce')) . '</a>';
                $content .= '</li>';
                continue;
            }
            $content .= $iconNoAvailable;
            $content .= ' <a href="https://my.mollie.com/dashboard/settings/profiles?utm_source=woocommerce&utm_medium=plugin&utm_campaign=partner" target="_blank">' . strtolower(__('Activate', 'mollie-payments-for-woocommerce')) . '</a>';
            $content .= '</li>';
        }
        $content .= '</ul></div>';
        $content .= '<div class="clear"></div>';
    ...
    protected function saveApplePaySettings()
    {
        ...
        $applepaySettings = [];
        isset($data['enabled']) && $data['enabled'] === '1' ? $applepaySettings['enabled'] = 'yes' : $applepaySettings['enabled'] = 'no';
        isset($data['display_logo']) && $data['display_logo'] === '1' ? $applepaySettings['display_logo'] = 'yes' : $applepaySettings['display_logo'] = 'no';
        isset($data['mollie_apple_pay_button_enabled_cart']) && $data['mollie_apple_pay_button_enabled_cart'] === '1' ? $applepaySettings['mollie_apple_pay_button_enabled_cart'] = 'yes' : $applepaySettings['mollie_apple_pay_button_enabled_cart'] = 'no';
        isset($data['mollie_apple_pay_button_enabled_product']) && $data['mollie_apple_pay_button_enabled_product'] === '1' ? $applepaySettings['mollie_apple_pay_button_enabled_product'] = 'yes' : $applepaySettings['mollie_apple_pay_button_enabled_product'] = 'no';
        isset($data['title']) ? $applepaySettings['title'] = $data['title'] : $applepaySettings['title'] = '';
        isset($data['description']) ? $applepaySettings['description'] = $data['description'] : $applepaySettings['description'] = '';
    ...
    protected function saveApiKeys($settings)
    {
        ...
        foreach ($settings as $setting) {
            if ($setting['id'] === $liveKeyName && $liveValueInDb) {
                if ($postedLiveValue === '**********') {
                    $_POST[$liveKeyName] = $liveValueInDb;
                } else {
                    $pattern = '/^live_\w{30,}$/';
                    $this->validateApiKeyOrRemove($pattern, $postedLiveValue, $liveKeyName);
                }
            } elseif ($setting['id'] === $testKeyName && $testValueInDb) {
                if ($postedTestValue === '**********') {
                    $_POST[$testKeyName] = $testValueInDb;
                } else {
                    $pattern = '/^test_\w{30,}$/';
                    $this->validateApiKeyOrRemove($pattern, $postedTestValue, $testKeyName);
                }
            }
        }
        return $settings;
    ...
    public function get_sections()
    {
        ...
        $sections = ['' => __('General', 'mollie-payments-for-woocommerce'), 'mollie_components' => __('Mollie Components', 'mollie-payments-for-woocommerce'), 'advanced' => __('Advanced', 'mollie-payments-for-woocommerce')];
        if ($isAppleEnabled) {
            $sections['applepay_button'] = __('Apple Pay Button', 'mollie-payments-for-woocommerce');
        }
    ...
    protected function isKlarnaEnabled(): bool
    {
        ...
        foreach ($klarnaGateways as $klarnaGateway) {
            if (array_key_exists('mollie_wc_gateway_' . $klarnaGateway, $this->registeredGateways) && array_key_exists($klarnaGateway, $this->paymentMethods) && $this->paymentMethods[$klarnaGateway]->getProperty('enabled') === 'yes') {
                $isKlarnaEnabled = \true;
                break;
            }
        }
    ...
    {
        $isBillieEnabled = array_key_exists('mollie_wc_gateway_billie', $this->registeredGateways) && array_key_exists('billie', $this->paymentMethods) && $this->paymentMethods['billie']->getProperty('enabled') === 'yes';
        if ($isBillieEnabled) {
