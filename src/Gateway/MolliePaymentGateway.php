...
use WP_Error;
class MolliePaymentGateway extends WC_Payment_Gateway implements \Mollie\WooCommerce\Gateway\MolliePaymentGatewayI
{
    /**
     * @var bool
     */
    ...
    public function __construct(PaymentMethodI $paymentMethod, PaymentService $paymentService, OrderInstructionsService $orderInstructionsService, MollieOrderService $mollieOrderService, Data $dataService, Logger $logger, NoticeInterface $notice, HttpResponse $httpResponse, MollieObject $mollieObject, PaymentFactory $paymentFactory, string $pluginId)
    {
        ...
        add_action('woocommerce_email_order_meta', [$this, 'displayInstructions'], 10, 3);
    ...
    {
        return $this->dataService->getPaymentMethod($this->paymentMethod->getProperty('id'));
    }
    /**
     * @return bool
     */
    ...
    {
        return in_array(get_woocommerce_currency(), $this->getSupportedCurrencies(), \true);
    }
    /**
     * @return array
     */
    ...
    {
        $default = ['AUD', 'BGN', 'BRL', 'CAD', 'CHF', 'CZK', 'DKK', 'EUR', 'GBP', 'HKD', 'HRK', 'HUF', 'ILS', 'ISK', 'JPY', 'MXN', 'MYR', 'NOK', 'NZD', 'PHP', 'PLN', 'RON', 'RUB', 'SEK', 'SGD', 'THB', 'TWD', 'USD'];
    ...
    public function handlePaidOrderWebhook($order, $payment)
    {
        ...
        $this->logger->debug(__METHOD__ . ' - ' . $this->id . ": Order {$order_id} does not need a payment by Mollie (payment {$payment->id}).", [\true]);
    }
    /**
    ...
    {
        $this->logger->debug($this->id . ': Could not process payment, order ' . $orderId . ' not found.');
        $this->notice->addNotice('error', sprintf(
            /* translators: Placeholder 1: order id. */
            __('Could not load order %s', 'mollie-payments-for-woocommerce'),
            $orderId
        ));
    ...
    {
        if ($admin_instructions && !$this::$alreadyDisplayedAdminInstructions || !$admin_instructions && !$this::$alreadyDisplayedCustomerInstructions) {
            ...
            $instructions = $this->orderInstructionsService->executeStrategy($this, $payment, $order, $admin_instructions);
        ...
        if ($admin_instructions && !$this::$alreadyDisplayedAdminInstructions) {
            $this::$alreadyDisplayedAdminInstructions = \true;
        }
        if (!$admin_instructions && !$this::$alreadyDisplayedCustomerInstructions) {
            $this::$alreadyDisplayedCustomerInstructions = \true;
        }
    }
    /**
     * @param      $title
    ...
    {
        $isPaymentApi = substr($order->get_meta('_mollie_order_id', \true), 0, 3) === 'tr_';
        $resource = $order->get_meta('_mollie_order_id', \true) && !$isPaymentApi ? 'orders' : 'payments';
        $this->view_transaction_url = 'https://my.mollie.com/dashboard/' . $resource . '/%s?utm_source=woocommerce&utm_medium=plugin&utm_campaign=partner';
    ...
    protected function gatewayHasFields(): void
    {
        ...
        $dropdownDisabled = $this->paymentMethod->hasProperty('issuers_dropdown_shown') && $this->paymentMethod->getProperty('issuers_dropdown_shown') === 'no';
        if ($dropdownDisabled) {
